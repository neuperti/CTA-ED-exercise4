...
set.seed(123L)
dfm_newstweets_results <- textmodel_wordfish(dfm_newstweets) # removed argument "sparse = TRUE"0
...
# compress the document-feature matrix at the newspaper level
dfm_newstweets <- dfm_group(kaneko_dfm_study1, groups = Newspaper)
...
#### estimate the Wordfish model ####
set.seed(123L)
kaneko_newstweets_results <- textmodel_wordfish(dfm_newstweets)

summary(kaneko_newstweets_results)

```
Interpretation of results:
The theta seems to be estimating the "right-wingness" of the newspapers with newspapers like Chugoku and Asahi being relatively left-wing while newspapers like Yomiuri and Nikkei fall more on the right. These results are broadly coherent with both the theta scores of the paper as well as their hard-coded expectations of political leanings.

2. Visualize the results
```{r}
## We can then plot our estimates of the thetas---i.e., the estimates of the latent Japanese newspaper position.

textplot_scale1d(kaneko_newstweets_results)

```
interpretation of the plot:
The plot reflects the data above (as it should). Nevertheless, I am somewhat critical of the concept validity of these wordfish models as even the theoretical concept of left and right is contested. Since this is an unsupervised model, it could very well just be picking up any other latent structure for scaling - such as the tabloid to newspaper spectrum above.
